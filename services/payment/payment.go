...
func NewPaymentService(repository repositories.IRepositoryRegistry, minio utilminio.IMinioClient, kafka kafka.IKafkaRegistry, midtrans clients.IMidtransClient) IPaymentService {
...
func (p *PaymentService) uploadToMinio(ctx context.Context, invoiceNumber string, pdf []byte) (string, error) {
	...
	url, err := p.minio.UploadFile(ctx, filename, contentType, pdf)
	if err != nil {
		return "", err
	}
